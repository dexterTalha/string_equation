...
extension GetValue on String {
  ...
  List<double?>? getValues(String conditions, Map<String, dynamic>? answer, {String? parentId}) {
    ...
    var map = conditions.split(this).map((e) => MathUtils(isDateFormula).putValueAndSolveExpression(e, answer, parentId: parentId).toString()).map((e) => double.parse(e));
...
extension RemoveSurrounding on String {
  ...
  bool containsDateOrToday() {
    final dateRegex = RegExp(r'\b\d{2}-\d{2}-\d{4}\b'); // Matches DD-MM-YYYY format
